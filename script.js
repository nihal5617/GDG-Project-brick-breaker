...
const Text = {
  ...
  draw: function (str = "") {
    ...
    else ctx.fillText(str, CANVAS_WIDTH / 2 - 150, CANVAS_HEIGHT / 2 + 60);
  },
};
const controller = {
  x: canvas1.width / 2,
  ...
  move: function (e) {
    ...
    else if (e.touches[0].clientX <= 0) this.x = 20 + this.radius;
...
const obstacle = {
  ...
  create: function () {
    for (let i = 0; i < LOC.length; i++) this.draw(LOC[i][0], LOC[i][1]);
  },
